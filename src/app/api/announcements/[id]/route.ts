...
// PATCH /api/announcements/[id] — editar (solo ADMIN para facilities/documents/works, cualquier miembro para marketplace)
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await db.announcements.update({ where: { id }, data: body });
...
// DELETE /api/announcements/[id] — soft delete (solo ADMIN)
